use copilot_stop_notif::{
    ...
    format_message, format_message_with_options, load_email_config,
    load_hook_input_from_reader, load_transcript_data, run_hook, run_notify,
...
fn test_load_transcript_data_supports_jsonl() {
    ...
    let payload =
        build_hook_payload(&json!({"cwd": "D:/workspace/copilot-stop-notif"}), Some(&transcript));
...
fn test_validate_transcript_path_keeps_default_roots_when_extra_roots_configured() {
    ...
    fs::write(&transcript_path, "{\"role\":\"assistant\",\"content\":\"ok\"}\n").unwrap();
...
fn test_validate_transcript_path_rejects_hard_linked_transcript() {
    ...
    fs::write(&source_path, "{\"role\":\"assistant\",\"content\":\"ok\"}\n").unwrap();
